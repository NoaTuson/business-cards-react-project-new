...
export default function MyCard({ card, onFavoriteChange }) {
 ...
 const addFavorite = () => {
        fetch(`https://api.shipap.co.il/cards/${card.id}/favorite?token=7cddfc3e-a309-11ee-beec-14dda9d4a5f0`, {
            method: 'PUT',
            headers: {
    ...
    const removeFavorite = () => {
        fetch(`https://api.shipap.co.il/cards/${card.id}/unfavorite?token=7cddfc3e-a309-11ee-beec-14dda9d4a5f0`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                // Include other headers as necessary
        ...
        .catch(error => {
            console.error('There has been a problem with removing favorite:', error);
        });
    };
